...
export default function App() {
  ...
  const fetchAiSystemPrompt = async () => {
    try {
      const data = await safeFetchJson('/api/pool/ai/system-prompt');
      if (data.prompt) setAiSystem(data.prompt);
    } catch (e) {}
  };
  ...
  const handleAddToSandbox = (block: any, category: string) => {
    if (sandboxBlocks.find(b => b.id === block && b.category === category)) {
      showAlert(`Bloco ${block} já está no sandbox.`, 'info');
      return;
    }
    setSandboxBlocks([...sandboxBlocks, { id: block, category }]);
    showAlert(`Bloco ${block} adicionado ao Sandbox.`, 'success');
  };

  const handleRemoveFromSandbox = (index: number) => {
    const newBlocks = [...sandboxBlocks];
    newBlocks.splice(index, 1);
    setSandboxBlocks(newBlocks);
  };

  const handleAnalyzeSandbox = async () => {
    if (sandboxBlocks.length < 2) return;
    setIsAnalyzingSandbox(true);
    setSandboxReport(null);
    try {
      const data = await safeFetchJson('/api/pool/audit/composition', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blocks: sandboxBlocks.map(b => ({ category: b.category, file: b.id })) })
      });
      setSandboxReport(data);
    } catch (err: any) {
      showAlert('Erro ao analisar composição: ' + err.message, 'error');
    } finally {
  ...
  const handleAiGenerate = async () => {
    ...
    setAiResult(null);
    try {
      const data = await safeFetchJson('/api/pool/ai/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: aiPrompt,
      ...
      showAlert(`Geração concluída via ${aiProvider} (${aiModel})`, 'success');
    } catch (err: any) {
      showAlert('Erro AI: ' + err.message, 'error');
    } finally {
  ...
  const handleCopyAiResult = () => {
    if (aiResult) {
      ...
      showAlert('Resultado copiado!', 'success');
    }
  };
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="flex dark:bg-slate-900 bg-slate-100/60 p-1.5 rounded-xl border dark:border-slate-850 border-slate-300 gap-1 overflow-x-auto">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all shrink-0 ${
              activeTab === 'sandbox' 
                ? 'bg-indigo-600 dark:text-white text-black shadow-md' 
                : 'dark:text-slate-400 text-slate-600 hover:dark:text-slate-200 text-slate-800 hover:dark:bg-slate-800 bg-slate-200/40'
            }`}
          >
            <Layers className="w-4 h-4" />
            🧩 Sandbox {sandboxBlocks.length > 0 && `(${sandboxBlocks.length})`}
          </button>
          <button
  ...
  <button
    onClick={() => setActiveTab('logs')}
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all shrink-0 ${
              activeTab === 'ai' 
                ? 'bg-purple-600 dark:text-white text-black shadow-md' 
                : 'dark:text-slate-400 text-slate-600 hover:dark:text-slate-200 text-slate-800 hover:dark:bg-slate-800 bg-slate-200/40'
            }`}
          >
        ...
        {activeTab === 'sandbox' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
               <div>
                 <h2 className="text-xl font-bold dark:text-slate-100 text-slate-900 tracking-tight flex items-center gap-2">
                   <Layers className="w-6 h-6 text-indigo-500" />
                   Sandbox de Orquestração
                 </h2>
                 <p className="text-sm text-slate-500">Combine múltiplos blocos para analisar sinergia, coesão e rotas de integração.</p>
               </div>
               <div className="flex gap-2">
                 <button 
                   onClick={() => setSandboxBlocks([])}
                   className="px-4 py-2 bg-slate-200 dark:bg-slate-800 hover:opacity-80 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold border border-slate-300 dark:border-slate-700 transition-all flex items-center gap-2"
                 >
                   <Trash2 className="w-3.5 h-3.5" /> Esvaziar
                 </button>
                 <button 
                   disabled={sandboxBlocks.length < 2 || isAnalyzingSandbox}
                   onClick={handleAnalyzeSandbox}
                   className={`px-6 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${
                     sandboxBlocks.length < 2 ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg'
                   }`}
                 >
                   {isAnalyzingSandbox ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
                   {isAnalyzingSandbox ? 'Analisando...' : 'Analisar Composição'}
                 </button>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               <div className="lg:col-span-1 space-y-4">
                 <div className="dark:bg-slate-900/40 bg-white border dark:border-slate-800 border-slate-200 p-4 rounded-2xl min-h-[400px]">
                   <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center justify-between">
                     Blocos Selecionados
                     <span className="bg-slate-200 dark:bg-slate-800 px-2 py-0.5 rounded text-[10px]">{sandboxBlocks.length}</span>
                   </h3>
                   ...
                   {sandboxBlocks.length === 0 ? (
                     <div className="flex flex-col items-center justify-center h-[300px] text-center space-y-3 opacity-40">
                       ...
                       <p className="text-xs text-slate-500">Nenhum bloco selecionado.<br/>Vá para a aba 'Legos' e adicione blocos aqui.</p>
                     </div>
                   ) : (
                     <div className="space-y-2">
                       {sandboxBlocks.map((b, idx) => (
                         <div key={`${b.category}-${b.id}-${idx}`} className="flex items-center justify-between p-3 dark:bg-slate-950/60 bg-slate-50 rounded-xl border dark:border-slate-800 border-slate-200 group">
                           <div className="flex items-center gap-3">
                             <div className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center">
                               <Box className="w-4 h-4 text-indigo-400" />
                             </div>
                             <div>
                               <p className="text-[11px] font-bold dark:text-slate-200 text-slate-800">{b.id.replace('.ts', '')}</p>
                               <p className="text-[9px] text-slate-500 uppercase font-mono tracking-tighter">{b.category}</p>
                             </div>
                           </div>
                           <button 
                             onClick={() => handleRemoveFromSandbox(idx)}
                             className="p-1.5 opacity-0 group-hover:opacity-100 hover:bg-red-500/10 hover:text-red-400 rounded transition-all text-slate-600"
                           >
                             <Trash2 className="w-3.5 h-3.5" />
                           </button>
                         </div>
               ...
               <div className="lg:col-span-2 space-y-6">
                 {!sandboxReport ? (
                   <div className="dark:bg-slate-900/20 bg-slate-50 rounded-2xl border border-dotted dark:border-slate-800 border-slate-300 h-full flex flex-col items-center justify-center p-12 text-center">
                     <Zap className="w-12 h-12 text-slate-300 dark:text-slate-700 mb-4" />
                     <h3 className="text-lg font-bold text-slate-400 dark:text-slate-600">Aguardando Análise</h3>
                     <p className="text-sm text-slate-500 max-w-sm mt-2">Selecione pelo menos 2 blocos para descobrir como eles podem ser combinados em uma solução poderosa.</p>
                   </div>
                 ) : (
                   <div className="space-y-6 animate-in zoom-in-95 duration-300">
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                       <div className="dark:bg-slate-900/60 bg-white p-4 rounded-2xl border dark:border-slate-800 border-slate-200">
                         <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Sinergia</span>
                         <div className="text-2xl font-black text-indigo-500 tracking-tighter">{sandboxReport.synergy_score}%</div>
                         <div className="w-full bg-slate-200 dark:bg-slate-800 h-1 mt-2 rounded-full overflow-hidden">
                           <div className="bg-indigo-500 h-full transition-all duration-1000" style={{ width: `${sandboxReport.synergy_score}%` }} />
                         </div>
                       </div>
                       <div className="dark:bg-slate-900/60 bg-white p-4 rounded-2xl border dark:border-slate-800 border-slate-200">
                         <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Coesão</span>
                         <div className="text-2xl font-black text-emerald-500 tracking-tighter">{sandboxReport.cohesion}%</div>
                         <div className="w-full bg-slate-200 dark:bg-slate-800 h-1 mt-2 rounded-full overflow-hidden">
                           <div className="bg-emerald-500 h-full transition-all duration-1000" style={{ width: `${sandboxReport.cohesion}%` }} />
                         </div>
                       </div>
                       <div className="dark:bg-slate-900/60 bg-white p-4 rounded-2xl border dark:border-slate-800 border-slate-200">
                         <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Complexidade</span>
                         <div className="text-2xl font-black text-amber-500 tracking-tighter">{sandboxReport.complexity}%</div>
                         <div className="w-full bg-slate-200 dark:bg-slate-800 h-1 mt-2 rounded-full overflow-hidden">
                           <div className="bg-amber-500 h-full transition-all duration-1000" style={{ width: `${sandboxReport.complexity}%` }} />
                         </div>
                       </div>
                     </div>

                     <div className="dark:bg-slate-900/60 bg-white p-6 rounded-2xl border dark:border-slate-800 border-slate-200 space-y-4 shadow-sm">
                       <h3 className="text-xs font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-widest">Plano de Orquestração</h3>
                       <p className="text-sm dark:text-slate-300 text-slate-700 leading-relaxed italic border-l-2 border-indigo-500 pl-4">
                         &quot;{sandboxReport.orchestration_plan}&quot;
                       </p>
                     </div>

                     <div className="dark:bg-slate-950/40 bg-red-50/10 p-6 rounded-2xl border dark:border-slate-800 border-red-100 space-y-4">
                       <h3 className="text-xs font-bold text-red-500 uppercase tracking-widest">Alertas de Integração</h3>
                       <div className="space-y-2">
                         {sandboxReport.potential_issues?.map((issue: string, idx: number) => (
                           <div key={idx} className="flex items-start gap-3 p-3 bg-red-500/5 rounded-xl border border-red-500/10">
                             <AlertTriangle className="w-4 h-4 text-red-500 mt-0.5" />
                             <p className="text-xs text-red-800 dark:text-red-300/80">{issue}</p>
                           </div>
                         ))}
                       </div>
                     </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">Orquestração multi-provedor (Local & Cloud) para geração e auditoria de blocos.</p>
              </div>
              <div className="flex gap-2">
                 <button 
                 ...
                 <button 
                   ...
                   className={`px-6 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${
                     !aiPrompt.trim() || isAiGenerating ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg'
                   }`}
                 >
                   {isAiGenerating ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
                   {isAiGenerating ? 'Gerando...' : 'Executar Prompt'}
                 </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              <div className="lg:col-span-1 space-y-4">
                <div className="dark:bg-slate-900/40 bg-white border dark:border-slate-800 border-slate-200 p-4 rounded-2xl">
                  <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Configuração do Modelo</h3>
                  
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">Provedor</label>
                      <select 
                        value={aiProvider}
                        onChange={(e) => {
                          const p = e.target.value;
                          setAiProvider(p);
                          setAiModel(aiProviderOptions[p][0]);
                        }}
                        className="w-full bg-slate-100 dark:bg-slate-950 border dark:border-slate-800 border-slate-300 rounded-lg px-3 py-2 text-xs dark:text-slate-200 focus:outline-none focus:border-purple-500 transition-all"
                      >
                        {Object.keys(aiProviderOptions).map(p => (
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select 
                        ...
                        className="w-full bg-slate-100 dark:bg-slate-950 border dark:border-slate-800 border-slate-300 rounded-lg px-3 py-2 text-xs dark:text-slate-200 focus:outline-none focus:border-purple-500 transition-all"
                      >
                        {aiProviderOptions[aiProvider]?.map((m: string) => (
                          <option key={m} value={m}>{m}</option>
                        ))}
                        {aiProvider === 'ollama' && <option value="custom">Outro (Ollama Local)</option>}
                      </select>
                    </div>

                    <div className="pt-4 border-t dark:border-slate-800 border-slate-200">
                      <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-2">Instrução de Sistema</h4>
                      <textarea 
                        value={aiSystem}
                        onChange={(e) => setAiSystem(e.target.value)}
                        placeholder="Default Pool Prompt..."
                        className="w-full h-32 bg-slate-100 dark:bg-slate-950 border dark:border-slate-800 border-slate-300 rounded-lg p-3 text-[10px] font-mono dark:text-slate-400 focus:outline-none focus:border-purple-500 transition-all resize-none"
                      />
                    </div>
                    ...
                    <div className="p-3 bg-blue-500/5 rounded-xl border border-blue-500/10">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-bold text-blue-400 uppercase">Dica</span>
                      </div>
                      <p className="text-[9px] text-slate-500">Ollama requer uma instância local rodando na porta 11434 por padrão.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-3 space-y-6">
                <div className="dark:bg-slate-900/40 bg-white border dark:border-slate-800 border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
                   <h3 className="text-xs font-bold text-purple-500 dark:text-purple-400 uppercase tracking-widest">Painel de Prompt</h3>
                   <textarea 
                     value={aiPrompt}
                     onChange={(e) => setAiPrompt(e.target.value)}
                     placeholder="Descreva o bloco Lego que deseja gerar ou uma consulta técnica complexa..."
                     className="w-full h-40 bg-slate-50 dark:bg-slate-950 border dark:border-slate-850 border-slate-300 rounded-xl p-4 text-sm dark:text-slate-200 focus:outline-none focus:border-purple-500 transition-all shadow-inner"
                   />
                </div>

                {aiResult ? (
                  <div className="dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-200 rounded-2xl overflow-hidden shadow-lg animate-in zoom-in-95 duration-300">
                    <div className="flex items-center justify-between px-6 py-4 border-b dark:border-slate-800 border-slate-200 bg-slate-50 dark:bg-slate-950/40">
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 rounded-full bg-emerald-500" />
                        <span className="text-xs font-bold dark:text-slate-200 text-slate-800 uppercase tracking-widest">Resultado da Geração</span>
                      </div>
                      <div className="flex gap-2">
                        <button 
                          onClick={handleCopyAiResult}
                          className="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 hover:opacity-80 text-slate-700 dark:text-slate-300 rounded-lg text-[10px] font-bold transition-all flex items-center gap-2"
                        >
                          <CheckCircle2 className="w-3 h-3" /> Copiar Código
                        </button>
                      </div>
                    </div>
                    <div className="p-0 bg-slate-950 overflow-x-auto min-h-[300px]">
                      <pre className="p-6 font-mono text-[11px] text-emerald-400/90 leading-relaxed whitespace-pre">
                        <code>{aiResult}</code>
                      </pre>
                    </div>
                  </div>
                ) : isAiGenerating ? (
                   <div className="h-64 dark:bg-slate-900/20 bg-slate-50 rounded-2xl border border-dotted dark:border-slate-800 border-slate-300 flex flex-col items-center justify-center p-12 text-center">
                     <RefreshCw className="w-12 h-12 text-purple-500 animate-spin mb-4" />
                     <h3 className="text-lg font-bold dark:text-slate-600 text-slate-400">Processando Pensamento...</h3>
                     <p className="text-sm text-slate-500 max-w-sm mt-2">O motor está consultando o provedor {aiProvider} para sintetizar sua requisição.</p>
                   </div>
                ) : (
                  <div className="h-64 dark:bg-slate-900/20 bg-slate-50 rounded-2xl border border-dotted dark:border-slate-800 border-slate-300 flex flex-col items-center justify-center p-12 text-center">
                    <Cpu className="w-12 h-12 text-slate-300 dark:text-slate-700 mb-4" />
                    <h3 className="text-lg font-bold text-slate-400 dark:text-slate-600">Aguardando Prompt</h3>
                    <p className="text-sm text-slate-500 max-w-sm mt-2">Escolha um provedor e descreva o que deseja criar ou analisar na Pool.</p>
                  </div>
                )}
              </div>
            </div>
                      ...
                      {inspectingBlock.interopMatrix?.map((m: any, idx: number) => (
                        <div key={idx} className="bg-slate-900/40 border border-slate-800 p-3 rounded-xl space-y-2 relative overflow-hidden group">
                          ...
                          <div className="grid grid-cols-2 gap-1.5 mt-1">
                            ...
                            <div className="bg-slate-950/60 p-1.5 rounded border border-slate-800/60">
                              <span className="block text-[7px] text-slate-500 uppercase tracking-widest font-bold">Similaridade</span>
                              <span className="text-[9px] text-blue-400 font-semibold">{m.similarity || 0}%</span>
                            </div>
                            <div className="bg-slate-950/60 p-1.5 rounded border border-slate-800/60">
                              <span className="block text-[7px] text-slate-500 uppercase tracking-widest font-bold">Sinergia</span>
                              <span className="text-[9px] text-indigo-400 font-semibold">{m.synergy_potential || 0}%</span>
                            </div>
                            <div className="bg-slate-950/60 p-1.5 rounded border border-slate-800/60">
                              <span className="block text-[7px] text-slate-500 uppercase tracking-widest font-bold">Origem</span>
                              <span className="text-[9px] text-slate-300 font-semibold">{m.origin_compatibility ? 'OK' : 'DIFF'}</span>
                            </div>
                            <div className="bg-slate-950/60 p-1.5 rounded border border-slate-800/60">
